...
import { getPermissions, addRole } from "../../services/roles";
...
function AddRoleModal({isOpen, setIsOpen, title, subtitle, selectedPerms, setSelectedPerms, fetchRoles, addNotification}){
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedPerms.length === 0 || !newRoleName) {
        ...
        try {
            ...
            if(selectedPerms.includes("Admin") && selectedPerms.length > 1){
                const newPerms = selectedPerms.filter((item) => item === "Admin");

                response = await addRole(newRoleName, newPerms);
            } else{
                response = await addRole(newRoleName, selectedPerms);
            }
                    ...
                    <InputContent>
                        <InputLabel>
                            <EditName>
                                ...
                                <TextInput inputValue={newRoleName} setValue={setNewRoleName} type={"text"} placeholder={"Digite o nome do novo cargo"}/>
                            </EditName>
                        </InputLabel>
                    </InputContent>
